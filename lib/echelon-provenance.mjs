...
/** Default binding for `followup.create`: the invoking actor originates the follow-up. */
export const followupCreateOperations = () => ["created"];

/**
 * Reference receiver (REG-PROV-005..REG-PROV-010). `state` is `{ records: { [operationId]: record } }`.
 * `binding` is `{ system, capability, operations?: () => string[], recordsTransformation?: boolean }`.
...
export const receive = (state, { envelope, payload }, binding) => {
  ...
  const withInvoker = appended.ok ? appended.block : carried;
  const invoker = appended.ok ? { recorded: true, key: call.key, changed: appended.changed } : { recorded: false, reason: appended.error };
